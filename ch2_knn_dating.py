...
def classifyPerson():
    labels = ['in no way at all', 'in small doses', 'in large doses']
    print("Enter the following stuff about the guy you wanna check out:")
    numFlyer = float(input("Number of Frequent Flyer Miles Earned Per Year = "))
    perVid = float(input("Percentage of Time Spent Playing Video Games = "))
    litIce = float(input("Liters of Ice-Cream Consumed Per Week = "))
    X, y = file2matrix("./data/datingTestSet_withNumericLabels.txt")
    normX, minVals, ranges = autonorm_intuitive(X)
    inX = np.array([numFlyer, perVid, litIce])
    norm_inX = (inX - minVals) / ranges
    prediction = classify0(norm_inX, normX, y, 4)
    print("You'll probably like the person %s." % (labels[prediction - 1]) )